#pragma once
#include "simulator.hpp"

namespace sjtu {

void Calculate(std::vector<Matrix *> keys, std::vector<Matrix *> values,
               Rater &rater, GpuSimulator &gpu_sim,
               MatrixMemoryAllocator matrix_memory_allocator) {
  assert(keys.size() == values.size());

  Matrix *K_T = nullptr;

  for (size_t i = 0; i < keys.size(); ++i) {
    size_t n = i + 1;
    auto current_query = rater.GetNextQuery(); // in HBM

    // Transpose keys[i] in HBM: shape becomes (512, 1)
    gpu_sim.Transpose(keys[i], kInGpuHbm);

    if (i == 0) {
      K_T = keys[0];
    } else {
      Matrix *new_KT = matrix_memory_allocator.Allocate("KT_" + std::to_string(i));
      gpu_sim.Concat(K_T, keys[i], new_KT, 1, kInGpuHbm);
      if (i > 1) {
        gpu_sim.ReleaseMatrix(K_T);
      }
      K_T = new_KT;
    }

    std::vector<Matrix *> ans_rows;

    for (size_t r = 0; r < n; ++r) {
      // 1. Get row r of current_query in HBM: shape (1, 512)
      Matrix *q_row = matrix_memory_allocator.Allocate("qr_" + std::to_string(r));
      gpu_sim.GetRow(current_query, r, q_row, kInGpuHbm);

      // 2. Compute q_row @ K_T over 512 channels
      // q_elem: (1, 1), kt_row: (1, n) -> prod: (1, n)
      Matrix *qk_row_acc = nullptr;
      for (size_t c = 0; c < 512; ++c) {
        Matrix *q_elem = matrix_memory_allocator.Allocate("qe_" + std::to_string(r) + "_" + std::to_string(c));
        gpu_sim.GetColumn(q_row, c, q_elem, kInGpuHbm);
        gpu_sim.MoveMatrixToSharedMem(q_elem);

        Matrix *kt_row = matrix_memory_allocator.Allocate("kr_" + std::to_string(r) + "_" + std::to_string(c));
        gpu_sim.GetRow(K_T, c, kt_row, kInGpuHbm);
        gpu_sim.MoveMatrixToSharedMem(kt_row);

        Matrix *prod = matrix_memory_allocator.Allocate("qkp_" + std::to_string(r) + "_" + std::to_string(c));
        gpu_sim.MatMul(q_elem, kt_row, prod);
        gpu_sim.ReleaseMatrix(q_elem);
        gpu_sim.ReleaseMatrix(kt_row);

        if (c == 0) {
          qk_row_acc = prod;
        } else {
          Matrix *new_acc = matrix_memory_allocator.Allocate("qk_acc_" + std::to_string(r) + "_" + std::to_string(c));
          gpu_sim.MatAdd(qk_row_acc, prod, new_acc);
          gpu_sim.ReleaseMatrix(qk_row_acc);
          gpu_sim.ReleaseMatrix(prod);
          qk_row_acc = new_acc;
        }
      }
      gpu_sim.ReleaseMatrix(q_row);

      // 3. Softmax on qk_row_acc (shape 1, n)
      Matrix *exp_row = matrix_memory_allocator.Allocate("sexp_" + std::to_string(r));
      gpu_sim.MatExp(qk_row_acc, exp_row);
      gpu_sim.ReleaseMatrix(qk_row_acc);

      Matrix *sum_val = matrix_memory_allocator.Allocate("ssum_" + std::to_string(r));
      gpu_sim.Sum(exp_row, sum_val);

      Matrix *soft_row = matrix_memory_allocator.Allocate("ssoft_" + std::to_string(r));
      gpu_sim.MatDiv(exp_row, sum_val, soft_row);
      gpu_sim.ReleaseMatrix(exp_row);
      gpu_sim.ReleaseMatrix(sum_val);

      // 4. Compute ans_row = soft_row @ V (shape 1, 512)
      Matrix *ans_row_acc = nullptr;
      for (size_t k = 0; k < n; ++k) {
        Matrix *s_elem = matrix_memory_allocator.Allocate("se_" + std::to_string(r) + "_" + std::to_string(k));
        gpu_sim.GetColumn(soft_row, k, s_elem, kInSharedMemory); // (1, 1)

        gpu_sim.MoveMatrixToSharedMem(values[k]); // (1, 512)

        Matrix *prod = matrix_memory_allocator.Allocate("vp_" + std::to_string(r) + "_" + std::to_string(k));
        gpu_sim.MatMul(s_elem, values[k], prod); // (1, 512)
        gpu_sim.ReleaseMatrix(s_elem);
        gpu_sim.MoveMatrixToGpuHbm(values[k]);

        if (k == 0) {
          ans_row_acc = prod;
        } else {
          Matrix *new_acc = matrix_memory_allocator.Allocate("ar_acc_" + std::to_string(r) + "_" + std::to_string(k));
          gpu_sim.MatAdd(ans_row_acc, prod, new_acc);
          gpu_sim.ReleaseMatrix(ans_row_acc);
          gpu_sim.ReleaseMatrix(prod);
          ans_row_acc = new_acc;
        }
      }
      gpu_sim.ReleaseMatrix(soft_row);

      // Move ans_row_acc to HBM
      gpu_sim.MoveMatrixToGpuHbm(ans_row_acc);
      ans_rows.push_back(ans_row_acc);
    }

    gpu_sim.ReleaseMatrix(current_query);

    // 5. Assemble Ans in HBM by concatenating ans_rows vertically
    Matrix *ans = ans_rows[0];
    for (size_t r = 1; r < n; ++r) {
      Matrix *next_ans = matrix_memory_allocator.Allocate("ans_hbm_" + std::to_string(r));
      gpu_sim.Concat(ans, ans_rows[r], next_ans, 0, kInGpuHbm);
      if (r > 1) {
        gpu_sim.ReleaseMatrix(ans);
      }
      gpu_sim.ReleaseMatrix(ans_rows[r]);
      ans = next_ans;
    }

    gpu_sim.Run(false, &matrix_memory_allocator);
    rater.CommitAnswer(*ans);
  }
}

void Test(Rater &rater, GpuSimulator &gpu_sim,
          MatrixMemoryAllocator &matrix_memory_allocator) {
  Calculate(rater.keys_, rater.values_, rater, gpu_sim,
            matrix_memory_allocator);
  rater.PrintResult(gpu_sim);
}

} // namespace sjtu
